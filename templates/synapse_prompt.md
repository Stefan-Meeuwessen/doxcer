<!-- SYNAPSE TEMPLATE -->
Analyseer deze Synapse (.json) Notebook en schrijf in Markdown documentatie voor onze Azure DevOps Wiki omgeving:

1. De opgeleverde dim of fact in twee tabellen;
    - Functioneel (Voor de BI experts)
    - Technisch (Voor de Data Engineers)
    - Mermaid UML (Functionele data flow met Synapse-specifieke componenten)
...
## 🏗️ Synapse Architectuur Context
{Beschrijf de positie binnen de Synapse workspace}
{Beschrijf integratie met Synapse SQL Pools (dedicated/serverless)}
{Beschrijf gebruik van ADLS Gen2 storage}
...
| ------------------------- | ----------------- | ------------- | -------------------------- | ----------------------------- | ------------------------- | ---------------------------------------------------------------------------------- |
| voorbeeld: `dim_project_fk` | voorbeeld: string | voorbeeld: Ja | voorbeeld: ADLS-Container  | voorbeeld: staging.dim_project | voorbeeld: dim_project_sk | voorbeeld: F.col("dim_project_sk").cast("string").alias("dim_project_fk"),         |
|                           |                   |               |                            |                               |                           |                                                                                    |
...
Deze documentatie is automatisch gegenereerd op basis van de Synapse notebooklogica en dient als startpunt voor review door Data Engineering en BI. Eventuele aanvullingen, correcties of optimalisaties kunnen direct in deze Wiki worden doorgevoerd.

<p align="center">
🚀 <em>Samen zorgen we voor consistente, uitlegbare en onderhoudbare data-producten in Azure Synapse Analytics.</em>
</p>